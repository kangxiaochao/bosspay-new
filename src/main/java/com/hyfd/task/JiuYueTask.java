...
public class JiuYueTask {
	...
	private static Logger log = Logger.getLogger(JiuYueTask.class);

	@Scheduled(fixedDelay = 60000)
	public void queryYuanTeOrder() {
		 Map<String, Object> map = new HashMap<String, Object>();
		try {
			String id="2000000055";															
	        ...
	        for (Map<String, Object> order : orderList){
	    		...
	    		if(result != null && !(result.equals(""))) {
	    			...
	    			if(status.equals("2")) {
	    				...
	    				flag = 0;
	    				log.debug("玖玥充值失败：" + orderId + ":" + utilsMap.get("serialno")+"--"+status+statusDesc);
	    			}
	    		}
	    		map.put("status", flag);
