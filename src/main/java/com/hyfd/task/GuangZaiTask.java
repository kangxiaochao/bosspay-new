...
@Component
public class GuangZaiTask {

	private static Logger log = Logger.getLogger(GuangZaiTask.class);
		...
		try {
			...
			for (Map<String, Object> order : orderList) {
				...
				if(!orderId.equals("") && orderId != null) {
					...
					log.info("光载查询返回"+dataJson);
					String statCode = dataJson.getString("status"); // 交易状态码(0或者1处理中 2成功3失败 4充值锁定中 (也是处理中) 9未确认)
