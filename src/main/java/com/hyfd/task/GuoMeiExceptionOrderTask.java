...
@Component
public class GuoMeiExceptionOrderTask {
	...
	private static Logger log = Logger.getLogger(GuoMeiExceptionOrderTask.class);
	...
	public void queryGuoMeiOrder() {
		...
		try {
			...
			List<Map<String, Object>> orderList = exOrderDao.selectByTask(param);
			for (Map<String, Object> order : orderList) {
				...
				String queryResult = sendChargeQuery(queryUrl, secretKey, serialNumber, timestamp, provinceCode,
						cityCode, fromType, orgType, applyChlId, operId, orderId);
	...
	public boolean moveOrder(Map<String, Object> order) {
		...
		int a = exOrderDao.deleteByPrimaryKey(id);// 将订单从异常订单删除
		if (a > 0) {
	...
	public static Map<String, String> getQueryResultCode(String responseStr) {
		...
		try {
			...
			if (null != resultJson.getJSONObject("payApplyLog")
					&& !"".equals(resultJson.getJSONObject("payApplyLog"))) {
