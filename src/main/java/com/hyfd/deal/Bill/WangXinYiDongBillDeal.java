...
public class WangXinYiDongBillDeal implements BaseDeal {
    ...
    public Map<String, Object> deal(Map<String, Object> order) {
        ...
        try {
            ...
            log.error("发起网信移动,请求参数" + parameter);
            //发送httppost请求
            String data = ToolHttp.post(false, linkUrl, parameter,"text/plain");
            log.error("网信移动充值提交完成，返回信息为" + data);
            if (null != data && !data.equals("")){
                ...
                // 请求超时,未获取到返回数据
                flag = -1;
                String msg = "网信移动充值,号码[" + phonenum + "],金额[" + spec + "(分)],请求超时,接收到返回数据为null";
                map.put("resultCode", msg);
                log.error(msg);
            }
        }catch(Exception e) {
            log.error("网信移动充值方法出错" + e + MapUtils.toString(order));
        }
        map.put("status", flag);
        log.error(map);
        return map;
