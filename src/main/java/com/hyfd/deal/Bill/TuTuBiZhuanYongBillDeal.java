...
public class TuTuBiZhuanYongBillDeal implements BaseDeal{

	private static Logger log = Logger.getLogger(TuTuBiZhuanYongBillDeal.class);
	...
	public Map<String, Object> deal(Map<String, Object> order) {
		...
		String cookies = "";
		if(cookie != null){
			log.error("兔兔币专用获取的Cookie为"+MapUtils.toString(cookie));
			...
			if(x == 1){
				...
				flag = -1;
				map.put("resultCode", "兔兔币专用充值出现异常，网络连接问题，或者兔兔币返回{code:8888,msg:服务器淘气了，我们的工程师正在修理它...}");
			}
		}else{
			flag = 4;
	...
	public static int transferX(String transferUrl,String phone,String money,String transferCookies){
		...
		try {
			...
			log.debug("兔兔币专用充值返回-->"+transferResult);
			...
			if(transferCode == 0){
				flag = 1;
				log.debug("兔兔币专用充值成功  ---- "+phone + "|"+transferCode+msg);
			}else if(transferCode == 8888){
				flag = -1;
			}else{
				log.error("兔兔币专用充值方法   ---- "+phone + "|"+transferCode+msg);
			}
		} catch (HttpException e) {
			flag = -1;
			log.error("兔兔币专用充值方法报错   --HttpException"+e);
			e.printStackTrace();
		} catch (IOException e) {
			flag = -1;
			log.error("兔兔币专用充值方法报错   --IOException"+e);
			e.printStackTrace();
		}finally{
