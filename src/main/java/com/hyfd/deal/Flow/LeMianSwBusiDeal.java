...
public class LeMianSwBusiDeal implements BaseDeal
{
    private static Logger log = Logger.getLogger(LeMianSwBusiDeal.class);
    ...
    public Map<String, Object> deal(Map<String, Object> order)
    {
        ...
        try
        {
            ...
            if (orderData == null)
            {
                String myMsg = "乐免省网[流量充值]请求超时,手机号[" + phoneNo + "],流量包[" + spec + "MB]";
                log.error(myMsg);
            }
            else
            {
                ...
                {
                    flag = 0;
                    log.error("乐免省网[流量充值]请求:提交失败!status[" + status + "]手机号[" + phoneNo + "],流量包[" + spec + "MB]");
                }
                else
                {
                    flag = 1;
                    // 充值成功处理
                    // log.error("乐免省网[流量充值]请求:提交成功!手机号["+phoneNo+"],流量包["+spec+"MB]");
                }
            }
        }
        ...
        {
            log.error("乐免省网[流量充值]出错" + e.getMessage() + "||" + MapUtils.toString(order));
        }
        map.put("status", 3);
        return map;
    }
