...
public class QianMiProvinceFlowDeal implements BaseDeal
{
    
    private static Logger log = Logger.getLogger(QianMiProvinceFlowDeal.class);
    ...
    public Map<String, Object> deal(Map<String, Object> order)
    {
        ...
        try
        {
            ...
            String token = paramMap.get("tokenP");
            String appKey = paramMap.get("appKeyP");//
            String appSecret = paramMap.get("appSecretP");// 密钥
            String pid = "0";// (String)order.get("providerProductId");
        ...
        {
            log.error("千米流量充值出错" + e.getMessage() + "|||" + MapUtils.toString(order));
        }
        map.put("status", 3);
        return map;
    }
