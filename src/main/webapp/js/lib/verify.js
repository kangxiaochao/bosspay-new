...
var platform_verify = function() {
	...
	/**
	 * 正double数，可验证>=0 && <=99999999.99 的数字
	 * @param str
	 * @returns
	 */
	var doubleZ = function(str){
	...
	/**
	 * 正double数，可验证0-9.999  的数字
	 * @param str
	 * @returns
	 */
	var doubleT = function(str){
	...
	var inputDataVali = function (inputNode, nodeType){
		...
		if(null != vType && (type == "text" || type == "password")){// || inputNode.type == "hidden"
			...
			if(value.length != 0){
				...
				}else if(vType == "doubleT"){//小于10的正浮点数
					resultArr = doubleT(value);
					
				}else if(vType == "chinaLetterNumber"){//中文字母数字
					resultArr = chinaLetterNumber(value);
					
				}else if(vType == "doubleZ"){//double：三位小数点
					resultArr = doubleZ(value);
