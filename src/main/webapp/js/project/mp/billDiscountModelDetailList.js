...
$(function() {
	...
	$(myJqTbId).jqGrid({
		url : basePath + 'dataDiscountModelDetailList/'+id,
		mtype : "GET",
		datatype : "json",
		caption : "代理商折扣模板详情列表", //设置表标题
		page : 1,
...
function search(){
	...
	$(myJqTbId).jqGrid('setGridParam',{ 
        url:basePath + 'dataDiscountModelDetailList/'+id,
        postData:{'pname':pname,'provinceCode':provinceCode}, //发送数据 
        page:1 
...
function edit(){
	var id=$('#modelId').val();
	var myBasePath = basePath+"dataDiscountModelDetailEditPage/"+id;
