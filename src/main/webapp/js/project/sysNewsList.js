...
$(function() {
	...
	$(myJqTbId).jqGrid({
		url : basePath + 'sysNews',
		mtype : "GET",
		datatype : "json",
		caption : "公告列表", // 设置表标题
		page : 1,
		colNames : [ 'id', '标题', '显示顺序', '更新时间', '发布时间'],
		colModel : [ {
			name : 'id',
			key : true,
			sortable : false,
	...
	setMyActive(1, 7); // 设置激活页

	message();
});

function add() {
	location.href = basePath + "sysNewsAddPage";
}

function edit(id) {
	var myBasePath = basePath + 'sysNewsEditPage/' + id;
...
function del(id) {
	layer.confirm('确认要删除吗?', {skin: 'layui-layer-molv',icon: 3, title:'提示'}, function(index){
		var myDelUrl = basePath + 'sysNews/' + id;
		$.ajax({
			type : 'DELETE',
			url : myDelUrl,
			data : {
