...
<c:import url="../head.jsp" />
 ...
 <body>
	<div id="wrapper">
			...
			<div class="row wrapper border-bottom white-bg page-heading">
				<div class="col-lg-10">
					...
					<ol class="breadcrumb">
						...
						<li><a>系统</a></li>
						<li class="active"><strong>话费批量修改充值结果</strong></li>
					</ol>
				</div>
					...
					<form id="fileForm" name="fileForm"  method="post" action="${basePath }batchQueryFilterPhone" enctype="multipart/form-data" style="width: 1px; height: 1px;">
	      			...
	      			<form id="editForm"  method="POST"  role="form" class="form-horizontal">
	      				...
	      				<div class="form-group">
							<div class="col-sm-1">
								<input type="radio" id="succ" name="stauts" value="3"/>
								<label for="succ" style="color:green">充值成功</label>
							</div>
							<div class="col-sm-1">
								<input type="radio" id="fail" name="stauts" value="4"/>
								<label for="fail" style="color:red">充值失败</label>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-2 control-label"><B>代理商订单号</B></label>
							<div class="col-sm-7">
								<textarea class="form-control" id="agentOrderId" name="agentOrderId" oninput="lockSubmitButton()"
					   			 placeholder="多条请用','分隔" vMin="11" cols=40 rows=6></textarea>
							</div>
							
							<div class="col-sm-2">
								<input type="button" class="btn btn-success" id="importTxt" name="importTxt" value="TXT导入"/>
								<br/><br/>
								<input type="button" class="btn btn-danger" id="orderCheck" name="orderCheck" value="单号检测"/>
							</div>
						...
						<div class="form-group">
							...
							<label id="countNumMsg" class="col-sm-4 msg-label-con"></label>
						</div>
						<div class="form-group">
							<div class="col-sm-12 col-sm-offset-5">
								<button type="button" id="queryButtion" class="btn btn-primary" onclick="submitOrder()">
									立即修改<i class="fa fa-arrow-circle-right"></i>
								</button>
							</div>
						</div>
					</form>
		...
		<script type="text/javascript" charset="UTF-8" src="${basePath }js/project/mp/batchModifyOrderStatus.js"></script>
</body>
</html>